...
$filex = '../../vids.txt';
$chnk = <<<EOF
[ ["./intro.mp4", "1280", "720", "0:21", "2488023", "945270.13677812", "30"]
EOF;
file_put_contents($filex, $chnk);
$filex = '../../ctr.txt';
$chnk = "";
file_put_contents($filex, $chnk);
  // Reset counter
...
if (isset($_REQUEST['filename'])) {
	...
	if ($handle = opendir($listdirectory)) {
		...
		foreach ($DirectoryContents as $dirname => $val) {
	     ...
	     // Write to txt files
$filex = '../../vids.txt';
$d = $fileinfo['playtime_string'];
$h = $fileinfo['video']['resolution_y'];
$w = $fileinfo['video']['resolution_x'];
$br = $fileinfo['bitrate'];
$si = $fileinfo['filesize'];
$fr = $fileinfo['video']['frame_rate'];
$a = <<<EOD
,["./video/
EOD;
$b = <<<EOD
...
"]
EOD;
$t = $a.$filename.$b.$w.$b.$h.$b.$d.$b.$si.$b.$br.$b.$fr.$c;
file_put_contents($filex, $t, FILE_APPEND);		
      // Count    
$ctr = ($ctr + 1);
$filex = '../../ctr.txt';
file_put_contents($filex, $ctr);			
